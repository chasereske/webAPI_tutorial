...
router.post('/:userId/shoppingcart/:productId', async (req, res) => { try {
...
router.put('/:userId/shoppingcart/:productId', async (req, res) => { try {
...
router.delete('/:userId/shoppingcart/:productId', async (req, res) => { try {
    const user = await User.findById(req.params.userId);
    if (!user) return res.status(400).send(`The user with id "${req.params.userId}" does not exist.`);
